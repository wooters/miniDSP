 ...
 *  for the given string.  Characters are 5 columns wide with 1-column
 *  spacing.  Rows are numbered 0 (top) to 6 (bottom). */
static int pixel_at(const char *text, unsigned col, unsigned row)
{
    unsigned len = (unsigned)strlen(text);
    unsigned char_width = 6;  /* 5 data columns + 1 spacing column */
                             ...
                             double duration_sec, double sample_rate)
{
    ...
    unsigned len = (unsigned)strlen(text);
    unsigned grid_cols = len * 6 - 1;  /* 5 data + 1 space per char, minus trailing space */
    ...
    for (unsigned c = 0; c < grid_cols; c++) {
        ...
        for (unsigned r = 0; r < grid_rows; r++) {
            int on_now  = pixel_at(text, c, r);
            int on_prev = (c > 0) ? pixel_at(text, c - 1, r) : 0;
            int on_next = (c + 1 < grid_cols) ? pixel_at(text, c + 1, r) : 0;
